...
type JJ11hhOpusEncoderDecoder struct {
	...
	encodedFrameBuffer EncodedFrame
	...
	encodedFrameReturnBuffer []EncodedFrame

	decoder                *opus.Decoder
	decodedFrameBuffer     PCMFrame
	decodedFrameBufferTail int
}

func newJJ11hhEncoderDecoder(
	sampleRate int,
	numChannels int,
	frameDuration OPUSFrameDuration,
	bufferSafetyFactor int,
) (*JJ11hhOpusEncoderDecoder, error) {
	...
	return &JJ11hhOpusEncoderDecoder{
		...
		pcmFrameBuffer:           make(PCMFrame, bufferSize),
		encodedFrameBuffer:       make(EncodedFrame, bufferSize),
		pcmFrameBufferHead:       0,
		pcmFrameBufferTail:       0,
		encodedFrameBufferTail:   0,
		encodedFrameReturnBuffer: make([]EncodedFrame, bufferSafetyFactor),
		decoder:                  decoder,
		decodedFrameBuffer:       make(PCMFrame, bufferSize),
		decodedFrameBufferTail:   0,
	}, nil
}

func (encdec JJ11hhOpusEncoderDecoder) String() string {
	return fmt.Sprintf("Type: JJ11hh, Sample Rate: %d, Num Channels: %d, Frame Duration %d", encdec.sampleRate, encdec.numChannels, encdec.frameDuration)
}

func (encdec JJ11hhOpusEncoderDecoder) GetFrameDuration() time.Duration {
